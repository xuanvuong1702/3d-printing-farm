...
@pytest.fixture()
def reports_db_path(tmp_path) -> str:
    db_path = str(tmp_path / "test_reports_service.db")
    run_migrations(db_path)
    return db_path

@pytest.fixture()
def conn(reports_db_path: str) -> Iterator[sqlite3.Connection]:
...
@pytest.fixture()
def insert_printer(reports_db_path: str) -> Callable[..., int]:
    _counter = {"n": 0}

    def _insert(
        *,
        name: str = "Reports Test Printer",
...
@pytest.fixture()
def insert_job(
    reports_db_path: str, insert_printer: Callable[..., int]
) -> Callable[..., int]:
