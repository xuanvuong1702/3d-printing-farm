"""
Unit test cho `app/moonraker/http_client.py` (chunk C2, E0-3).

Nguyên tắc (theo state + CLAUDE.md):
- Chỉ mock tầng transport HTTP (dùng `respx`, thư viện mock chính thức
  cho `httpx`). KHÔNG mock các hàm nội bộ (`_map_print_stats_state`,
  `get_status`, ...) - test phải đi qua logic map/công thức thật.
- Input/output mong đợi lấy từ "Phụ lục: Checklist kỹ thuật Moonraker"
  trong `Decisions.md` (endpoint, field multipart, STATE_MAP, công thức
  timeRemaining, ngưỡng pct > 0.02).
"""
...
def _objects_query_result(
    *,
    ...
    """Dựng payload JSON giả lập `GET /printer/objects/query`."""
    print_stats: dict = {}
...
def test_get_status_offline_when_webhooks_not_ready_overrides_printing():
    """webhooks.state != 'ready' -> luôn OFFLINE, bất kể print_stats.state."""
    respx.get(f"{BASE_URL}/printer/objects/query").respond(
        200,
        json=_objects_query_result(
...
def test_get_status_offline_when_webhooks_not_ready_overrides_error():
    """Kiểm tra thêm với 1 raw_state khác 'printing' để chắc chắn override
    áp dụng cho mọi print_stats.state, không riêng gì printing."""
    respx.get(f"{BASE_URL}/printer/objects/query").respond(
        200,
        json=_objects_query_result(
...
def test_get_status_progress_at_exact_boundary_no_time_remaining():
    """pct == 0.02 (không > 0.02) -> có progress_percent, KHÔNG có
...
def test_get_status_progress_zero():
    """pct == 0 -> progress_percent == 0, không có time_remaining."""
    respx.get(f"{BASE_URL}/printer/objects/query").respond(
        200,
        json=_objects_query_result(
            webhooks_state="ready",
...
def test_get_status_progress_above_threshold_computes_time_remaining():
    """pct > 0.02 -> tính cả progress_percent lẫn time_remaining_seconds
    theo công thức elapsed * (1 - pct) / pct."""
    pct = 0.25
    elapsed = 100.0
    respx.get(f"{BASE_URL}/printer/objects/query").respond(
        200,
...
def test_check_if_printing_false_when_offline():
    """webhooks chưa ready -> OFFLINE -> không tính là đang in dù
    print_stats.state báo printing."""
    respx.get(f"{BASE_URL}/printer/objects/query").respond(
        200,
        json=_objects_query_result(
            webhooks_state="startup", print_stats_state="printing"
...
def test_request_omits_api_key_header_when_none():
    """D-003: api_key là cột nullable - không gửi header nếu không có."""
    route = respx.get(f"{BASE_URL}/server/info").respond(
        200, json={"result": {"klippy_connected": True}}
    )
